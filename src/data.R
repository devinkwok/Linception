...
fit_linear_models = function(data_index, dataframe, num_folds) {
    ...
    num_cols = length(predictors)
    inclusion_matrix = combination_matrix(num_cols)
    #TODO: change this to create a graph of the predictors
    #then get a list of all the nodes from the graph
    ...
    # iterate through nodes of graph and k-folds for each model
    for (i in 1:length(inclusion_matrix[,1])) {

        predictor_matrix = inclusion_matrix[i,]
        ...
        for (j in 1:num_folds) {
            ...
            model_data_row = list(
                ...
                "predictors"=paste(predictor_matrix, collapse=""),
                "k_fold"=data_folds[["fold_num"]][[j]]
                )
    ...
    #TODO: iterate through edges of graph and get pair statistics
    # for (edge in list_pgraph_edges(graph)) {
    #     get_paired_statistics()
    # }
...
generate_lms = function(filenames, num_folds) {
    linear_models = NULL
    
    for (i in 1:length(filenames)) {
        name = filenames[i]
        dataset = load_dataframe(name)
        if (is.null(dataset)) {
...
generate_meta_data = function(data_dir) {
    # get files
    filenames = list_data_filenames(data_dir, EXTENSION)
    filenames = filenames[1:MAX_DATASETS]  # truncate
    for (i in 1:length(filenames)) {
        filenames[[i]] = file.path(data_dir, filenames[[i]])
    }

    # generate data
    lms_and_data = generate_lms(filenames, NUM_FOLDS)
    ...
    metafile_name = timestamp("lm-data-individual", EXTENSION)
